...
class _FakeEngine:
    ...
    def suspend(self):
        """Model the battery-only lifecycle used by the pause coordinator."""
        return None
...
def test_admitted_generation_finishes_on_pinned_engine_during_explicit_swap():
    """A generation admitted before a swap boundary is allowed to finish on A."""
    engine_a = _FakeEngine()
    holder = S.EngineHolder(engine_a, load_kwargs={})
    engine_b = _FakeEngine()
    engine_b.model_id = "EngineB"
    swap_started = threading.Event()

    def blocked_until_lease_releases(**_kwargs):
        swap_started.set()
        holder._engine = engine_b
        return holder.status()

    holder._swap_now = blocked_until_lease_releases
    httpd = ThreadingHTTPServer(("127.0.0.1", 0), S.make_handler(holder, api_key=None))
    threading.Thread(target=httpd.serve_forever, daemon=True).start()
    ...
    try:
        headers = (f"POST /v1/chat/completions HTTP/1.1\r\n"
                   ...
                   "Connection: close\r\n\r\n").encode()
        split = len(body) // 2
        sock.sendall(headers + body[:split])
        deadline = time.monotonic() + 2.0
        while holder._lifecycle._generation != 1 and time.monotonic() < deadline:
            time.sleep(0.005)
        assert holder._lifecycle._generation == 1

        def load_b():
            try:
                admin_result["body"] = _post(
                    f"http://127.0.0.1:{address[1]}", "/admin/load", {"model": "B"})
            except Exception as exc:  # noqa: BLE001 - diagnostic on assertion failure
                admin_result["error"] = exc

        admin = threading.Thread(target=load_b)
        admin.start()
        deadline = time.monotonic() + 2.0
        while holder.lifecycle_snapshot()["state"] != "reloading" \
                and time.monotonic() < deadline:
            time.sleep(0.005)
        assert holder.lifecycle_snapshot()["state"] == "reloading"
        assert admin.is_alive()
        ...
        payload = json.loads(response.read(content_length))
        assert payload["model"] == "FakeModel"
        assert engine_a.calls
        admin.join(2.0)
        assert not admin.is_alive()
        assert "error" not in admin_result
        assert holder.current is engine_b
...
def test_inventory_reports_no_loaded_model_when_holder_read_is_closed(holder_server):
    """The model-free inventory remains a 200 while a resident engine is unavailable
    for short-read acquisition during a lifecycle replacement."""
    holder, base = holder_server
    assert holder.current is not None
    with holder._lifecycle._lock:
        holder._lifecycle._read_open = False
    inv = _get(base, "/admin/models")
    assert inv["loaded"] is None
...
def test_machine_model_less_holder_never_dynamically_delegates(monkeypatch):
    holder = S.EngineHolder(None, load_kwargs={})
    holder._lifecycle._read_open = False

    def unexpected_dynamic_access(_self, name):
        raise AssertionError(f"dynamic holder access without a lease: {name}")

    monkeypatch.setattr(S.EngineHolder, "__getattr__", unexpected_dynamic_access)
    httpd = ThreadingHTTPServer(("127.0.0.1", 0), S.make_handler(holder, api_key=None))
    base = f"http://127.0.0.1:{httpd.server_address[1]}"
    thread = threading.Thread(target=httpd.serve_forever, daemon=True)
    thread.start()
    try:
        machine = _get(base, "/machine")
        assert machine["model"] is None
        assert set(machine) >= {"chip", "bandwidth", "memory"}
    finally:
        httpd.shutdown()
...
def _paused_fixture(state="ready", start_worker=True):
    ...
    if state == "paused":
        ...
        pause._test_lease = holder.acquire_generation()  # fixture-owned admitted request
        assert pause._test_lease is not None
...
def test_explicit_ac_reload_is_not_reported_as_battery_pause():
    """The generic reloading phase must keep ordinary AC load semantics even when
    the optional BatteryPause adapter is configured."""
    holder, pause, base, httpd = _paused_fixture(start_worker=False)
    started = threading.Event()
    release = threading.Event()

    def blocked_swap(**_kwargs):
        started.set()
        assert release.wait(2.0)
        holder._engine = _CloseableEngine()
        return holder.status()

    holder._swap_now = blocked_swap
    result = {}
    request = threading.Thread(
        target=lambda: result.setdefault("body", _post(
            base, "/admin/load", {"model": "org/Explicit"})))
    request.start()
    try:
        ...
        for path in ("/v1/models", "/metrics"):
            with pytest.raises(urllib.error.HTTPError) as error:
                _get(base, path)
            assert error.value.headers.get("X-MLX-Dspark-Unavailable-Reason") is None
            body = json.loads(error.value.read())
            assert body["error"]["code"] == 503
            assert "battery" not in body["error"]["message"].lower()
    finally:
        release.set()
        request.join(2.0)
        pause.stop()
...
def test_admin_load_finishing_after_battery_is_reconciled_and_unloaded():
    """A holder-owned explicit swap finishing after the battery edge is followed
    by exactly one automatic suspension of the resulting resident model."""
    holder = S.EngineHolder(_CloseableEngine(), load_kwargs={})
    pause = S.BatteryPause(holder, log=lambda msg: None)
    swap_started = threading.Event()
    ...
    try:
        ...
        # The explicit finalizer queues the deferred unload; execute that existing
        # worker step deterministically rather than relying on timing.
        assert pause.state == "unloading"
        holder._lifecycle._step()
        health = pause.health()
...
def test_battery_pause_headers_track_the_lifecycle_phase():
    ...
    try:
        ...
        admitted = _holder.acquire_generation()
