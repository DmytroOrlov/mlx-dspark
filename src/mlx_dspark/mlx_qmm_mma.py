...
actually wins — the result is cached under ~/.cache/mlx_dspark/prism_qmm_mma/.
``MLX_DSPARK_NO_PRISM_QMM_MMA`` disables the kernel;
``MLX_DSPARK_PRISM_QMM_RECAL`` forces a fresh hardware probe.
...
_CACHE_DIR = os.path.expanduser("~/.cache/mlx_dspark/prism_qmm_mma")
...
def calibrate(*models, verbose: bool = False) -> dict:
    ...
    cached: dict = {}
    if os.path.isfile(path) and not _flag("MLX_DSPARK_PRISM_QMM_RECAL"):
        try:
            with open(path) as f:
                cached = json.load(f)
        except Exception:  # noqa: BLE001
...
def install(*models, verbose: bool = False) -> bool:
    ...
    if _flag("MLX_DSPARK_NO_PRISM_QMM_MMA"):
        return False
    try:
